...
function elLeaveUsed(empId, type, from, to) {
  ...
  elHolidays.forEach(function(h){
    if(h.fields['Confirmed']) confirmedDates[h.fields['Date']] = true;
  });
  var total = 0;
  elRecords.forEach(function(r){
    ...
    if(to   && dt > to)   return;
    // Don't count confirmed bank holidays against annual leave
    if(type === 'Annual' && confirmedDates[d]) return;
...
function elShowTab(tab) {
  elActiveTab = tab;
  ['annual','sick','wfh','ticket'].forEach(function(t){
...
function renderTabContent(tab) {
  ...
  if(tab==='all'){
    var allE=elRecords.filter(function(r){return elEmpId(r.fields['Employee'])===elCurrentEmpId;})
      .sort(function(a,b){return new Date(b.fields['Date'])-new Date(a.fields['Date']);});
    ...
    var rows=allE.map(function(r){var t=r.fields['Type']||'—';
      return '<tr style="border-bottom:1px solid var(--bdr)">'
        +'<td style="padding:8px 12px;font-size:12px">'+elFmtDate(r.fields['Date'])+'</td>'
        +'<td style="padding:8px 12px;font-weight:600;color:'+(tc[t]||'var(--txt)')+'">'+e(t)+'</td>'
        +'<td style="padding:8px 12px;text-align:center;font-family:monospace">'+r.fields['Days']+'</td>'
        +'<td style="padding:8px 12px;color:var(--txt2);font-size:12px">'+e(r.fields['Notes']||'')+'</td>'
        +'<td><button onclick="openEditEntry(\''+r.id+'\')" style="background:none;border:1px solid var(--bdr2);border-radius:4px;padding:2px 8px;cursor:pointer;font-size:11px">Edit</button></td>'
      +'</tr>';}).join('');
    el.innerHTML='<div style="display:flex;justify-content:flex-end;margin-bottom:8px">'
      ...
      +(rows?'<table style="width:100%;border-collapse:collapse;font-size:13px">'
        +'<thead><tr style="background:var(--bg2)"><th style="padding:8px 12px;text-align:left">Date</th>'
        +'<th style="padding:8px 12px;text-align:left">Type</th><th style="padding:8px 12px;text-align:center">Days</th>'
        +'<th style="padding:8px 12px;text-align:left">Notes</th><th></th></tr></thead><tbody>'+rows+'</tbody></table>'
  ...
  var entries = elRecords.filter(function(r){
    ...
    var d = r.fields['Date']; if(!d) return false;
    var dt=new Date(d); dt.setHours(0,0,0,0);
  ...
  if(tab==='annual') {
    ...
    if(bhInPeriod.length) {
      ...
      bholNote = '<div style="background:var(--amber-bg);border:1px solid var(--amber-bdr);border-radius:6px;padding:8px 12px;font-size:12px;margin-bottom:12px;color:var(--amber)">'+
        ...
        ' fall within this period — these are excluded from used days.</div>';
    }
  }
  ...
  if(!entries.length) {
    ...
    listHtml = '<table style="width:100%;border-collapse:collapse;font-size:13px">'+
      ...
      '<th style="padding:8px 12px;text-align:left;font-weight:600;color:var(--txt2)">Date</th>'+
      '<th style="padding:8px 12px;text-align:center;font-weight:600;color:var(--txt2)">Days</th>'+
      '<th style="padding:8px 12px;text-align:left;font-weight:600;color:var(--txt2)">Notes</th>'+
      '<th style="padding:8px 12px"></th>'+
      '</tr></thead><tbody>'+
      entries.map(function(r){
        var isBH = tab==='annual' && elHolidays.some(function(h){ return h.fields['Confirmed']&&h.fields['Date']===r.fields['Date']; });
        var style = isBH ? 'color:var(--txt3);font-style:italic' : '';
        return '<tr style="border-bottom:1px solid var(--bdr)">'+
          '<td style="padding:8px 12px;'+style+'">'+elFmtDate(r.fields['Date'])+(isBH?' 🏖️':'')+'</td>'+
          '<td style="padding:8px 12px;text-align:center;font-family:monospace;font-weight:600;'+style+'">'+r.fields['Days']+'</td>'+
          '<td style="padding:8px 12px;color:var(--txt2);'+style+'">'+e(r.fields['Notes']||'')+'</td>'+
          '<td style="padding:8px 12px"><button onclick="openEditEntry(\''+r.id+'\')" style="background:none;border:1px solid var(--bdr2);border-radius:4px;padding:2px 8px;cursor:pointer;font-size:11px">Edit</button></td>'+
        '</tr>';
      }).join('')+
      '</tbody></table>';
  }
  ...
  el.innerHTML = periodNavHtml + summaryHtml + bholNote +
    ...
    '<div style="border:1px solid var(--bdr2);border-radius:6px;overflow:hidden">'+listHtml+'</div>';
}
...
function elUpdatePreview() {
  ...
  txt.textContent = msg;
  prev.style.display = '';
  if(btn) btn.textContent = days.length>1 ? 'Save ('+days.length+' entries)' : 'Save';
}
function openAddEntry(type) {
  elCurrentEditId = null;
  document.getElementById('el-entry-title').textContent = 'Add Leave Entry';
  // Switch to range mode for adding
  var rangeF  = document.getElementById('el-range-fields');
  var singleF = document.getElementById('el-single-field');
  if(rangeF)  rangeF.style.display  = 'contents';
  if(singleF) singleF.style.display = 'none';
...
function openEditEntry(id) {
  ...
  elCurrentEditId = id;  // set regardless so delete works even if record not in cache
  var f = r ? r.fields : {};
  document.getElementById('el-entry-title').textContent = 'Edit Leave Entry';
  // Switch to single-date mode
  var rangeF  = document.getElementById('el-range-fields');
  var singleF = document.getElementById('el-single-field');
  if(rangeF)  rangeF.style.display  = 'none';
  if(singleF) singleF.style.display = 'block';
  document.getElementById('el-entry-type').value  = f['Type']||'Annual';
  document.getElementById('el-entry-date').value  = (f['Date']||'').substring(0,10);
  document.getElementById('el-entry-days').value  = String(f['Days']||'1');
...
async function saveLeaveEntry() {
  ...
  if(elCurrentEditId) {
    ...
    var date = document.getElementById('el-entry-date').value;
    if(!date){ toast('Date required','err'); return; }
    var fields = {'Employee':[elCurrentEmpId],'Type':type,'Date':date,'Days':days};
  ...
  // ADD MODE — date range, create one record per working day
  ...
  try {
    ...
    var allRecords = workDays.map(function(date){
      var f = {'Employee':[elCurrentEmpId],'Type':type,'Date':date,'Days':days};
      if(notes) f['Notes']=notes;
      return {fields:f};
    });
    for(var i=0; i<allRecords.length; i+=10) {
      var chunk = allRecords.slice(i, i+10);
      var res = await fetch(WORKER_URL+'/leave-records',
        {method:'POST', headers:getHeaders(), body:JSON.stringify({records:chunk})});
      if(!res.ok) throw new Error('HTTP '+res.status);
      var data = await res.json();
      (data.records||[]).forEach(function(r){ elRecords.push(r); });
    }
    document.getElementById('el-entry-modal').style.display='none';
    toast(workDays.length+' entr'+(workDays.length===1?'y':'ies')+' added','ok');
    await loadLeaveData();
    if(elCurrentEmpId) openEmpDetail(elCurrentEmpId);
  } catch(err){
    toast('Save failed: '+err.message,'err');
